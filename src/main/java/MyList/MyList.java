...
public class MyList<T> implements List<T>, Author{
    ...
    class MyIterator implements ListIterator<T> {
        ...
        public void remove() {
            ...
            MyList.this.remove(cursor); //удаляет значение последнего next или previous
            index--;
            cursor = -1;
        }
        ...
        public void set(T t) {
            ...
            MyList.this.set(cursor, t); //заменяет значение элемента array[cursor]
        }
        ...
        public void add(T t) {
            if (cursor == -1) {
                ...
                MyList.this.add(cursor, t); //добавляет элемент t по индексу cursor
            }
        }
    }
}
